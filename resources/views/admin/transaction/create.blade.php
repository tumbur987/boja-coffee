            ...
            <div class="form-group">
                ...
                <select class="form-control" id="table_id" name="table_id" required>
                    ...
                    @foreach ($tables as $table)
                        <option value="{{ $table->id }}" {{ old('table_id') == $table->id ? 'selected' : '' }}>Meja {{ $table->number }} ({{ $table->code }})</option>
                    @endforeach
                </select>
            </div>
                    ...
                    <div class="col-md-6">
                        <select name="items[0][product_id]" class="form-control" required>
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" data-price="{{ $product->price }}">{{ $product->name }} - Rp.{{ number_format($product->price) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
